...
const TeacherLayout: React.FC<TeacherLayoutProps> = ({ children, activePage }) => {
  ...
  if (authLoading || (authLoading && !user)) {
    return (
      <div className="min-h-screen flex flex-col">
        <NavBar />
        <div className="flex-1 flex items-center justify-center bg-greyed-white">
          <Loader />
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex bg-greyed-white pt-16">
        {/* Mobile menu overlay */}
        {showMobileMenu && isMobile && (
          <div
        ...
        <div className={`${
          isMobile
            ? `fixed inset-y-0 pt-16 left-0 z-50 transition-transform duration-300 ease-out ${
        ...
        <div className={`flex-1 transition-all duration-300 ${
          isMobile ? 'ml-0' : (sidebarCollapsed ? 'md:ml-14' : 'md:ml-48')
        }`}>
